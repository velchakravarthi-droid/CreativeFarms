...
export default function Home() {
  ...
  const blockNames = farmBlocks.map((block) => block.name);
  const stockNames = stockItems.map((item) => item.name);
      ...
      <section className="workspace">
        <header className="topbar">
          <div>
            ...
            <p>Production foundation with Supabase/PostgreSQL masters and Vercel-ready frontend.</p>
          </div>
          <div className="topbar-actions">
            <Pill tone="warn">Offline-first planned</Pill>
            <Pill>Supabase ready</Pill>
          </div>
        </header>

        <div className="content">
          <section id="dashboard" className="kpi-grid">
            <Card title="Blocks" action={<Pill tone="good">{farmBlocks.length} active</Pill>}>
              <strong className="metric">125 acres</strong>
              <p>Farm structure is block-first, then rows, then tree exceptions.</p>
            </Card>
            <Card title="Workers" action={<Pill>{workers.length} users</Pill>}>
              <strong className="metric">3 roles</strong>
              <p>Admin, Manager, and User are the current access masters.</p>
            </Card>
            <Card title="Stock" action={<Pill tone="warn">2 low</Pill>}>
              <strong className="metric">{stockItems.length} items</strong>
